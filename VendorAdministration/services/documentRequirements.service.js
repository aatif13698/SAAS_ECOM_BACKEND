...
const create = async (clientId, data, mainUser) => {
    try {
        ...
        const existing = await DocumentRequirement.findOne({ jobRole: data?.jobRole })

        if (existing) {
            throw new CustomError(statusCode.BadRequest || 500, `Document requirement for this role already exists`);
        }
...
const docRequiremtnByRole = async (clientId, roleId) => {
    try {
        const clientConnection = await getClientDatabaseConnection(clientId);
        const CustomField = clientConnection.model("documentCustomField", documentCustomFieldSchema);
        const DocumentRequirement = clientConnection.model("documentRequirement", documentRequirementSchema);

        const docRequiremnt = await DocumentRequirement.find({ jobRole: roleId });

        if (docRequiremnt.length == 0 || docRequiremnt.length > 1) {
            throw new CustomError(statusCode.BadRequest || 500, `Inavalid doc requirement found`);
        }


        const [fields] = await Promise.all([
            CustomField.find({ documentRequirementId: docRequiremnt[0]._id }),
        ]);
...
const getDocCustomData = async (clientId, userId) => {
    try {
        const clientConnection = await getClientDatabaseConnection(clientId);
        const DocData = clientConnection.model("documentCustomData", documentCustomDataSchema);
        const data = await DocData.findOne({userId: userId});
        if (!data) {
            throw new CustomError(statusCode.NotFound, "Doc data not found");
        }
        const customData = await DocData.findOne({ userId: userId });
        return customData;
    } catch (error) {
