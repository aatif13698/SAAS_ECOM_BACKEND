...
//         const existingSr = await SaleReturn.findOne({ srNumber: data?.srNumber }).lean();
//         if (existingSr) {
    ...
    try {
        const result = await session.withTransaction(async (session) => {
            ...
            if (data?.paidAmount > 0) {
                ...
                [pi] = await SaleReturn.create([{ ...data, receivedIn: [{ id: data.receivedIn, paymentType: "Payment", amount: Number(data.paidAmount) }], status: Number(data.balance) == 0 ? "paid" : "partially_paid" }], {
                    session,
                ...
                // ── Create Purchase Invoice ────────────────────────────
                [pi] = await SaleReturn.create([{ ...data, receivedIn: [], paymentMethod: "", status: "full_due" }], {
                    session,
                    ordered: true   // safe even for 1 document
                });
...
const list = async (clientId, filters = {}, options = { page: 1, limit: 10 }) => {
    try {
        ...
        const [saleReturns, total] = await Promise.all([
            ...
            ,
            SaleReturn.countDocuments(filters),
        ]);
        return { count: total, saleReturns };
    } catch (error) {
