    ...
    try {
        const result = await session.withTransaction(async (session) => {
            ...
            if (data?.paidAmount > 0) {
                ...
                [po] = await PaymentOut.create([{ ...data, type: "purchase_invoice", toLedger: data.supplierLedger, payedFrom: [{ id: data.payedFrom }], linkedId: linkedId }], {
                    session,
...
const createForSaleReturn = async (clientId, data, mainUser) => {
    const clientConnection = await getClientDatabaseConnection(clientId);
    const PurchaseInvoice = clientConnection.model('purchaseInvoice', purchaseInvoiceSchema);
...
const getById = async (clientId, id) => {
    try {
        ...
        let invoices = [];
        let user = null;
        if (paymentOut.type == "purchase_invoice") {
