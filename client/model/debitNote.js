...
const debitNoteSchema = new Schema(
    {
        ...
        dnNumber: { type: String, trim: true, required: true, unique: true, index: true }, // Unique for business integrity
        dnDate: { type: Date, default: Date.now, required: true }, // Stored as Date; frontend formats as needed
...
debitNoteSchema.index({ businessUnit: 1, poDate: -1 });
debitNoteSchema.index({ branch: 1, poDate: -1 });
debitNoteSchema.index({ supplier: 1, poDate: -1 });

// Virtual for total order amount (computed)
debitNoteSchema.virtual('totalOrderAmount').get(function () {
