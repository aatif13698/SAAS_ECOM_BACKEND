...
const salePerformaSchema = new Schema(
    {
...
salePerformaSchema.index({ businessUnit: 1, poDate: -1 });
salePerformaSchema.index({ branch: 1, poDate: -1 });
salePerformaSchema.index({ supplier: 1, poDate: -1 });

// Virtual for total order amount (computed)
salePerformaSchema.virtual('totalOrderAmount').get(function () {
    return this.items.reduce((sum, item) => sum + item.totalAmount, 0);
});

module.exports = salePerformaSchema;