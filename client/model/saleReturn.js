...
const saleReturnSchema = new Schema(
    {
        ...
        srNumber: { type: String, trim: true, required: true, unique: true, index: true }, // Unique for business integrity
        srDate: { type: Date, default: Date.now, required: true }, // Stored as Date; frontend formats as needed
...
saleReturnSchema.index({ businessUnit: 1, poDate: -1 });
saleReturnSchema.index({ branch: 1, poDate: -1 });
saleReturnSchema.index({ supplier: 1, poDate: -1 });

// Virtual for total order amount (computed)
saleReturnSchema.virtual('totalOrderAmount').get(function () {
    return this.items.reduce((sum, item) => sum + item.totalAmount, 0);
});

module.exports = saleReturnSchema;