...
const creditNoteSchema = new Schema(
    {
        ...
        cnNumber: { type: String, trim: true, required: true, unique: true, index: true }, // Unique for business integrity
        cnDate: { type: Date, default: Date.now, required: true }, // Stored as Date; frontend formats as needed
...
creditNoteSchema.index({ businessUnit: 1, poDate: -1 });
creditNoteSchema.index({ branch: 1, poDate: -1 });
creditNoteSchema.index({ supplier: 1, poDate: -1 });

// Virtual for total order amount (computed)
creditNoteSchema.virtual('totalOrderAmount').get(function () {
