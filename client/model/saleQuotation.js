...
const saleQuotationSchema = new Schema(
    {
...
saleQuotationSchema.index({ businessUnit: 1, poDate: -1 });
saleQuotationSchema.index({ branch: 1, poDate: -1 });
saleQuotationSchema.index({ supplier: 1, poDate: -1 });

// Virtual for total order amount (computed)
saleQuotationSchema.virtual('totalOrderAmount').get(function () {
    return this.items.reduce((sum, item) => sum + item.totalAmount, 0);
});

module.exports = saleQuotationSchema;