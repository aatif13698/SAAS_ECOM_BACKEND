        ...
        const result = categories.map(category => {
            const relatedSubcategories = subCategories
                ...
                .map((sub, index) => ({
                    ...
                    name: sub.name,
                    icon: sub.icon,
                    link: `/${category.name.replace(/\s+/g, '-').toLowerCase()}/${sub.name.replace(/\s+/g, '-').toLowerCase()}`
                }));
            ...
            return {
                ...
                name: category.name,
                icon: category.icon,
                ...(relatedSubcategories.length > 0 ? { submenu: relatedSubcategories } : { link: `/${category.name.replace(/\s+/g, '-').toLowerCase()}` })
            };
        });
...
exports.addNewAddress = async (req, res, next) => {
    try {
        ...
        const user = req.user;
        if (!clientId) {
            return res.status(httpStatusCode.BadRequest).send({ message: message.lblClinetIdIsRequired });
        }
        ...
        if (requiredFields.some((field) => !field)) {
            return res.status(httpStatusCode.BadRequest).send({ message: message.lblRequiredFieldMissing });
        }
        const clientConnection = await getClientDatabaseConnection(clientId);
...
exports.updateAddress = async (req, res, next) => {
    try {
        const { clientId, addressId, fullName, phone, alternamtivePhone, country, state, city, ZipCode, houseNumber, roadName, nearbyLandmark, address } = req.body;
        const user = req.user;
        if (!clientId) {
            return res.status(httpStatusCode.BadRequest).send({ message: message.lblClinetIdIsRequired });
        }
        if (!addressId) {
            return res.status(httpStatusCode.BadRequest).send({ message: message.lblAddressIdIdRequired });
        }
        const requiredFields = [fullName, phone, alternamtivePhone, country, state, city, ZipCode, houseNumber, roadName, nearbyLandmark, address];
        if (requiredFields.some((field) => !field)) {
            return res.status(httpStatusCode.BadRequest).send({ message: message.lblRequiredFieldMissing });
        ...
        if (!existingAddress) {
            return res.status(httpStatusCode.BadRequest).send({ message: message.lblAddressNotFound });
        }
...
exports.deleteAddress = async (req, res, next) => {
    try {
        ...
        if (!clientId) {
            return res.status(httpStatusCode.BadRequest).json({ message: message.lblClinetIdIsRequired });
        }
        if (!addressId) {
            return res.status(httpStatusCode.BadRequest).json({ message: message.lblAddressIdIdRequired });
        }
        const clientConnection = await getClientDatabaseConnection(clientId);
        const customerAddress = clientConnection.model('customerAddress', customerAddressSchema);
        const existingAddress = await customerAddress.findOne({ _id: addressId, deletedAt: null });
        if (!existingAddress) {
            return res.status(httpStatusCode.NotFound).json({ message: message.lblAddressNotFound });
        }
        await customerAddress.findByIdAndUpdate(addressId, { deletedAt: new Date() });
        return res.status(httpStatusCode.OK).json({
...
exports.getAddresses = async (req, res, next) => {
    try {
        ...
        if (!customerId) {
            return res.status(httpStatusCode.BadRequest).send({ message: message.lblCustomerIdIsRequired });
        }
        const clientConnection = await getClientDatabaseConnection(clientId);
        const clientUser = clientConnection.model('clientUsers', clinetUserSchema);
        const customerAddress = clientConnection.model('customerAddress', customerAddressSchema);
...
exports.getCart = async (req, res, next) => {
    try {
        ...
        if (userId) {
            ...
            cart = await Cart.findOne({ user: userId, status: "active" })
                .populate("items.productStock", "product priceOptions") // Optional: populate product details
                .lean(); // Return plain JS object for performance
        } else if (sessionId) {
            // Fetch cart for guest user
            cart = await Cart.findOne({ sessionId, isGuest: true, status: "active" })
                .populate("items.productStock", "product priceOptions")
                .lean();
        }
        if (!cart) {
            return res.status(httpStatusCode.NotFound).json({
